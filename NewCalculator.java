...
class NewCalculator  implements ActionListener		////////////클래스명변경
{
//클래스명 변경
//메뉴제거
//Grid 변경
...
//	JMenu menu3;
	...
	String[] strGrid={"7","8","9","+","4","5","6","-","1","2","3","*" ,"0","=","%","/" };	//////////Grid변경
	...
	public NewCalculator(){
		frame=new JFrame("계산기 예제");				/////////////////////타이틀 '계산기 예제'로 변경
//		menuBar=new JMenuBar();						/////////////////////메뉴제거
//
//		menu=new JMenu("보기(V)");
...
//		menu3.setFont(new Font("Sans-serif", 0, 12));
	...
	public void gui(){
//		menuBar.add(menu);		// 메뉴바에 메뉴들 1,2,3 붙였다 ///////////////메뉴제거
//		menuBar.add(menu2);	
//		menuBar.add(menu3);	
...
//		panelNorth.add(BorderLayout.NORTH,textfieldNorth);   // 패널에 텍스트필드를 두개 붙임.		///////////////////그림처럼 분할없애기
...
//		frame.setJMenuBar(menuBar);  				// 메뉴바 붙이기 		//////////////메뉴제거
		
		frame.setResizable(true);                                  //창 크기 변경 못하게 막는다.	////////창의 크기변경 가능하게 만듦
		//frame.setSize(220,310);
		frame.setSize(260,310);   							////////창의 크기 변경
	...
	public void actionPerformed(ActionEvent e){
		...
		try{	
			...
			if(str!="/" && str!= "*"&& str!= "-"&& str!= "+"&& str!= "%"&& str!= "="){	///////////숫자 판단에 C를 %로 변경
			...
			//연산기호가 들어왔을때 true.
			if(str=="/" || str== "*"|| str== "-"|| str== "+"|| str== "="|| str== "%"){ 		////////////연산기호에 % 추가
				...
				for(int a=0,h=1; a<ee.size(); a++,h++){   // ArrayList 크기만큼 반복
					...
					if(ve>0){							// 연산자 누적 모두 사용할때까지 반복.
						...
						if(store.get(a)=="+"){			// 연산자 + 일때
							...
							sum=sum*ee.get(h);
						}else if(store.get(a)=="%") {		///////////계산로직 % 추가 동작해야함
							try {
								sum=sum%ee.get(h);
							}catch(Exception exc) {
								sum=0;
			...
			//모든 값 초기화 시키고 다시 연산.			//////////////초기화 안 되는 부분 확인
			if(str=="C"){
				first="";
	...
	{
		NewCalculator cal=new NewCalculator();		////////////클래스명변경
		cal.gui();
	}// main()
}// class
